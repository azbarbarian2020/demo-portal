...
echo "================================================================="
echo "  Manufacturing West Demo Portal - Setup"
echo "  Key-Pair JWT Auth | SPCS Deployment"
echo "================================================================="
echo ""
...
" 2>/dev/null || echo "")
    ...
    if echo "$IP_LIST" | grep -q "$SPCS_CIDR"; then
        echo "  SPCS CIDR already in account policy."
        echo -e "${GREEN}✓ SPCS IP already allowed${NC}\n"
        return
    fi

    echo ""
    echo -e "  ${YELLOW}Account network policy does not include SPCS CIDR ($SPCS_CIDR).${NC}"
    echo -e "  ${CYAN}Adding SPCS CIDR to account policy...${NC}"
...
")
    ...
    echo -e "${GREEN}✓ SPCS CIDR added to account network policy${NC}\n"
}
...
" 2>/dev/null || echo "(endpoint not yet available)")
    ...
    echo ""
    echo "  To add demos:  Log in to the portal and click Admin"
